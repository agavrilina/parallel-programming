...
int	numtasks,              /* number of tasks in partition */
	...
	i, j, k, rc;           /* misc */
   //double a[N][N], b[N][N], c[N][N]; /* matrix A and B to be multiplied, result matrix C*/
   //double **a, **b, **c;
  // a = new double*[N];
   //b = new double*[N];
...
//fin = fopen("D:\\MatrixMult_MPI\times.txt", "w");
   ...
   if (taskid == MASTER)
   {
    ...
    for (i=0; i<N; i++)
    {
        ...
        //b[i] = new double[N];
         for (j=0; j<N; j++)
         {
            a[i][j] = RandomDouble(1, 6);
            b[i][j] = RandomDouble(2, 7);
         }
    }
